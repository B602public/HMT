...
<script type="text/javascript">
	$(function() {
		$('#admin_ret_table').datagrid({
			url : '${pageContext.request.contextPath}/order/orderManage_confirmDatagrid',
			fit : true,
			fitColumns : true,
			border : false,
			pagination : true,
			...
			function(){
				...
				$.ajax({
						...
						success:function(msg){
							$('#admin_ret_table').datagrid('load',msg);
						}
					}
				);
			}
	...
	function addDelFun(){
		$('<div/>').dialog({
			...
			onLoad:function(){
				//加载内容
				//$('#admin_ret_adddialog').form('load',rows[0]);
			}
		});
	...
	function searchDelFun() {
		$('#admin_ret_table').datagrid('load', {
			demandId : $('#admin_ret_layout input[name=demandId]').val()
		});
	}
	function clearDelFun() {
		$('#admin_ret_layout input[name=demandId]').val('');
		$('#admin_ret_table').datagrid('load', {});
	}
	function searchDelByDateFun(){
		$('#admin_ret_table').datagrid('load', {
			startDate : $('#admin_ret_layout input[name=startDate]').val(),
			endDate:$('#admin_ret_layout input[name="endDate"]').val()
		});
	}
	function showDelFun(){
		var rows = $('#admin_ret_table').datagrid('getChecked');
		if(rows.length == 1){
			$('<div/>').dialog({
				width:500,
				height:400,
...
<div id="admin_ret_layout" class="easyui-layout" data-options="fit:true" style="height:400px">
	...
	<div data-options="region:'center'">
		<table id="admin_ret_table" class="easyui-datagrid">
		</table>
	</div>
</div>
<div id="admin_ret_toolbar" >
	...
	<a href="javascript:void(0);" class="easyui-linkbutton" data-options="iconCls:'icon-edit',plain:true" style="float:left;" onclick="confrimDelFun()">编辑</a>
	<div class="datagrid-btn-separator"></div>
	<a href="javascript:void(0);" class="easyui-linkbutton" data-options="iconCls:'icon-add',plain:true" style="float:left;" onclick="addDelFun()">添加</a>
	<div class="datagrid-btn-separator"></div>
